  ...
  class mollie {
    ...
    public function sendSMS($message) {
      ...
      $result = $this->sendToHost('www.mollie.nl', '/xml/sms/',
					 ...
					 '&deliverydate=' . urlencode($this->deliverydate) .
...
// ----- Joep delete messagges via api
    public function deleteSMS($message) {

      $recipients = implode(',', $this->recipients);

      $result = $this->sendToHost('www.mollie.nl', '/xml/sms_cancel/',
                     'gateway=' . urlencode($this->gateway) .
                     '&username=' . urlencode($this->username) .
                     '&password=' . urlencode($this->password) .
                     '&originator=' . urlencode($this->originator) .
...
/*	
      ...
      $result = $this->sendToHost('www.mollie.nl', '/xml/sms_cancel/',
                     'username=' . urlencode($this->username) .
                     '&password=' . urlencode($this->password) .
                     '&reference=' . urlencode($this->reference)
                  );
...
// ----- Joep delete messagees via api	END --------- //
    ...
    protected function XMLtoResult_DEL ($xml) {
      $data = simplexml_load_string($xml);
	  
	  //var_dump($xml);
      $this->success       = ($data->result->success == 'true');
      $this->successcount  = $data->result->recipients;
